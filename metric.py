...
def metric(paths:list, preds:list, labels:list, m, needs=[]):
    """
    :param paths:
    :param preds:
    :param labels:
    ...
    myre = []
    if 'miou' in needs:
        r, rr = (0.5, m) if m > 0.5 else (m, 0.5)
        miou = 0.0
        for movie in moviePL.keys():
            pd, lb = moviePL[movie]
            pd_o = result2dict_n(r, pd)
            pd_r = result2dict_n(rr, pd)
            lb = result2dict_n(r, lb)
            iou = callMIOU_n(lb, pd_o, pd_r)
            miou += iou
        miou = miou / len(moviePL.keys())
        met.update({'mIoU': miou})

    if 'f1' in needs:
        f1 = callF1_n(m, moviePL)
        met.update({'F1': f1})
    return met, moviePL
...
def callMIOU(groundSceneDict, predSceneDict):
    ...
    return miou
def callMIOU_n(groundSceneDict, predSceneDict, predSceneDict_r):
    """
    Calculating Mean IoU for a movie
    :param groundSceneDict: dict, {shot_index: (start, end)}
    :param predSceneDict: dict, {shot_index: (start, end)}
    ...
    """
    ious_g = callIOU(groundSceneDict, predSceneDict)
    ious_p = callIOU(predSceneDict_r, groundSceneDict)
    miou_g = [iou for _, iou, _ in ious_g]
    miou_p = [iou for _, iou, _ in ious_p]
    miou = 0.5*np.mean(miou_g) + 0.5*np.mean(miou_p)\

    return miou
    #return miou, np.mean(miou_g), np.mean(miou_p)
