...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def _gen_rand_reason_link(self, links):
        ...
        return r_link
    #clue
    def _clue_matric(self, links:list, n_top=5):
        T = len(links)
        inxmat = np.zeros((T, n_top, T))
        for i, link in enumerate(links):
            for j in range(n_top):
            ...
            # inxmat[i, -1, i] = 1
        return inxmat
    def _build_graph_sclue(self, sclue, iclue, is_diag=True, is_norm=True):
        winsz = len(sclue)
        simgh = np.zeros((winsz, winsz))
        for i, link in enumerate(sclue):
            if len(link) != 0:
                #print(link)
                simgh[i, link] = 1
        # for i, link in enumerate(iclue):
        #         simgh[i][i] = 0
        if is_diag:
            simgh = np.eye(winsz) + simgh
        if is_norm:
    ...
    def _build_graph_iclue(self,  sclue, iclue, is_diag=True, is_norm=True):
        winsz = len(sclue)
        simgh = np.zeros((winsz, winsz))
        for i, link in enumerate(iclue):
            if len(link) != 0:
        ...
        if is_norm:
            simgh = self._norm_graph(simgh)
        return simgh
    def _build_graph_siclue(self, sclue, iclue, is_diag=True, is_norm=True):
        winsz = len(sclue)
        simgh = np.zeros((winsz, winsz))
        for i, link in enumerate(sclue):
            if len(link) != 0:
        ...
        if is_norm:
            simgh = self._norm_graph(simgh)
        return simgh
    def _build_graph_link_siclue(self, links: list, sclue, iclue, is_diag=True, is_norm=True):
        winsz = len(sclue)
        simgh = np.zeros((winsz, winsz))
        for i, link in enumerate(links):
            if len(link) != 0:
