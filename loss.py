...
def bce(pred, label):
    ...
    return loss
def bce_t(pred, label):
    batch_weight = weighter(label)
    loss = F.binary_cross_entropy(pred, label, batch_weight, reduction='none')
    # loss = F.binary_cross_entropy(pred, label)
    return loss
...
def pseudo_bce(pred, label):
    ...
    #ce_loss = F.binary_cross_entropy(pred, label, weight)
    reg = torch.mean(pred * pred)
    return ce_loss.mean() + 0.3*reg

def pseudo_bce_t(pred, label):
    weight = weighter(label, [0.5, 6])
    #weight = weighter(label)
    #ce_loss = F.binary_cross_entropy_with_logits(pred, label, weight, reduction='none')
    ce_loss = F.binary_cross_entropy(pred, label, weight)
    #reg = torch.mean(pred * pred)
    #return ce_loss.mean() + 0.1*reg
    return ce_loss
